...
Alpine.data("Challenge", () => ({
  ...
  async init() {
    highlight();
    await this.preloadSolvedSubmission();
  },
  ...
  async preloadSolvedSubmission() {
    if (!this.solvedByMe) {
      return;
    }

    this.isSolvedInputLocked = true;

    try {
      let response = await CTFd.pages.users.userSubmissions("me", this.id);
      let submissions = Array.isArray(response.data) ? response.data : [];

      let solvedSubmission = submissions.find(
        s => s.type === "correct" || s.type === "partial",
      );
      let fallbackSubmission = submissions.find(
        s => typeof s.provided === "string" && s.provided.length > 0,
      );

      if (solvedSubmission && solvedSubmission.provided) {
        this.submission = solvedSubmission.provided;
      } else if (fallbackSubmission && fallbackSubmission.provided) {
        this.submission = fallbackSubmission.provided;
      } else {
        this.submission = "flag submitted";
      }
    } catch (error) {
      if (!this.submission) {
        this.submission = "flag submitted";
  ...
  async renderSubmissionResponse() {
    ...
    if (this.response.data.status === "already_solved") {
      this.solvedByMe = true;
      await this.preloadSolvedSubmission();
    }
